defmodule RobotSimulatorTest do
  ...
  # test "create does something" do
  #   robot = RobotSimulator.create()
  #   assert robot == nil
  # end

  test "simulate" do
    assert RobotSimulator.create() |> RobotSimulator.simulate("?") == {:error, "invalid instruction"}
    robot = RobotSimulator.create() |> RobotSimulator.simulate("LRLRLRRA")
    assert RobotSimulator.position(robot) == {1, 0}
    assert RobotSimulator.direction(robot) == :east

    assert RobotSimulator.create() |> RobotSimulator.simulate("LRLRLRLURLRLRLALRLRLAL") == {:error, "invalid instruction"}
  end
