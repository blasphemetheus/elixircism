defmodule AnagramTest do
  ...
  test "letter frequency one " do
    assert Anagram.letter_frequency("a") == [{"a", 1}]
  end

  test "letter frequency none" do
    assert Anagram.letter_frequency("") == []
  end

  test "letter frequency order " do
    assert Anagram.letter_frequency("ab") == [{"a", 1}, {"b", 1}]
    assert Anagram.letter_frequency("ba") == [{"a", 1}, {"b", 1}]
    assert Anagram.letter_frequency("abaaaab") == [{"a", 5}, {"b", 2}]
  end

  test "anagram?" do
    refute Anagram.anagram_of?("sup", "guy")
    assert Anagram.anagram_of?("sup", "pus")
    assert Anagram.anagram_of?("suppy", "ypusp")
    assert Anagram.anagram_of?("lllls.uppy", "ylpulspll.")
    refute Anagram.anagram_of?("lllls.uppy", "ylpulsp ll.")
