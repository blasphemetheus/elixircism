defmodule SumOfMultiplesTest do
  ...
  test "multiple stuff" do
    assert SumOfMultiples.multiple_of?(2, 2) == true
    assert SumOfMultiples.multiple_of?(2, 3) == false
    assert SumOfMultiples.multiple_of?(2, 6) == true
    assert SumOfMultiples.multiple_of?(3, 30) == true
    assert SumOfMultiples.multiple_of?(4, 30) == false
  end

  test "multipleSSS stuff" do
    assert SumOfMultiples.multiples_of_any?(30, [3, 4]) == true
    assert SumOfMultiples.multiples_of_any?(30, [3, 5, 10, 30, 1]) == true
    
    assert SumOfMultiples.multiples_of_any?(4, [3, 5]) == false
    assert SumOfMultiples.multiples_of_any?(4, [3]) == false
  end
